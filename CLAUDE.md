...
- **Text in Postgres (self-hosted; reached through PostgREST with the `supabase-js` client — bundled
  ...
  `mcp_used_codes` `backup_state` `integration_keys` `activity_log` `analytics_events` `analytics_scroll` `redirects` `subscribers` — full DDL in
...
| Newsletter / subscribe / opt-in / SMTP not sending | `lib/subscribers.ts`, `lib/mail.ts` (Nodemailer; config on `integration_keys`), `api/subscribe`, `api/newsletter/*`, `api/mail`, `components/admin/NewsletterFields.tsx` | `docs/features.md` "Newsletter" |
| Broadcast not sent / re-sent / reply notify | `lib/broadcast.ts` (`broadcastDuePosts`, `posts.broadcast_at`), `api/cron` (publish tick + hourly), `lib/comment-notify.ts`, `lib/newsletter-email.ts` | `docs/features.md` "Newsletter" |
...
| Book reading mode / fullscreen 2-column / "Chế độ đọc sách" toggle | `components/blog/BookMode.tsx` (overlay + horizontal pagination), `features.bookMode`, `src/app/(blog)/[slug]/page.tsx` (`#post-body` + meta-line toggle), `globals.css` (`.book-*`) | `docs/features.md` "Book reading mode" |
...
| `subscribers.ts` / `mail.ts` | `addSubscriber`, `confirmSubscriber`, `unsubscribeByToken`, `getConfirmedSubscribers`, `listSubscribers` / `getSmtpConfig`, `saveSmtpConfig`, `sendMail`, `isMailConfigured` | Newsletter double opt-in (`subscribers` table, per-row token) + Nodemailer SMTP (config on `integration_keys`, server-only, env fallback). `sendMail` never throws — degrades when unconfigured |
| `broadcast.ts` / `comment-notify.ts` / `newsletter-email.ts` | `broadcastDuePosts` / `notifyReply` / `broadcastEmail`, `replyEmail` | Broadcast-on-publish: cron emails confirmed subscribers ONCE per newly-live post + stamps `posts.broadcast_at` (stamps even with no SMTP/subs → no back-catalogue burst; migration backfills live posts; `savePost` upsert omits the column so an edit never re-sends). Reply-notify: `after()` from the comment route emails the parent commenter. Email HTML = the pure escaped builders |
