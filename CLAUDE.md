...
| Newsletter / subscribe / opt-in / SMTP not sending / test send | `lib/subscribers.ts`, `lib/mail.ts` (Nodemailer; config on `integration_keys`), `api/subscribe`, `api/newsletter/*`, `api/mail` (+ `api/mail/test` = owner-only sample send), `components/admin/NewsletterFields.tsx`, `components/blog/Subscribe{Form,Trigger,Overlay}.tsx` | `docs/features.md` "Newsletter" |
