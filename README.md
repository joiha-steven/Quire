...
| 🔍&nbsp;**Reading** | instant local + Postgres full-text search · a left sidebar rail (categories + tags, or a post's ToC) · related posts · reading time · progress bar · full-bleed images on mobile · **book reading mode** — an opt-in fullscreen two-column "book" reader (paper + grain, drop cap, shareable `#read` link; desktop + iPad) |
...
| 📚&nbsp;**Series** | group posts into an ordered series · a series box on each part · `/series/[slug]` listing · admin **Series manager** (rename / reorder / remove across posts) |
