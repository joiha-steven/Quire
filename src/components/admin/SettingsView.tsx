...
export function SettingsView({ settings, presets, commentEnv, integrations, posts }: { settings: SiteSettings; presets: ThemePreset[]; commentEnv: CommentEnv; integrations: IntegrationStatus; posts: { slug: string; title: string }[] }) {
  ...
  return (
    <div className="pb-24">
      ...
      {/* Content: reading features + reader comments + WordPress import — all in the
          2-col grid so no card stretches full-width over a narrow form. */}
      {tab === 'content' && (
        <div className="grid items-start gap-5 xl:grid-cols-2">
      ...
      {/* Appearance: palette + custom CSS on the left, the type stack on the right —
          two balanced columns, nothing stretched full-width. */}
      {tab === 'appearance' && (
        <div className="grid items-start gap-5 xl:grid-cols-2">
