...
// Right-aligned menu button -> dropdown of configurable header links (pages,
// categories, custom URLs). Items are set in admin settings.
...
function MenuLink({ item, onNavigate }: { item: MenuItem; onNavigate: () => void }) {
  ...
  if (/^https?:\/\//.test(item.href)) {
    return (
      <a href={item.href} target="_blank" rel="noopener" className={cls} onClick={onNavigate}>
        {item.label}
      </a>
    )
  }
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-50 mt-2 w-52 overflow-hidden rounded-xl border border-neutral-200 bg-white py-1 shadow-lg dark:border-neutral-700 dark:bg-neutral-900">
            {items.map((item, i) => (
              <MenuLink key={`${item.href}-${i}`} item={item} onNavigate={() => setOpen(false)} />
            ))}
          </div>
        </>
      )}
