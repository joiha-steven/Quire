...
- [x] **Homepage mode, part 1: the post list or a chosen page.** Done 2026-07-31, see
      `docs/features.md` and `src/web/home-mode.ts`. `mode` is `list | page`; `front` joins the
      union in part 2 rather than sitting there rendering nothing.
      [ADR 0014](../docs/decisions/0014-homepage-modes.md).
      The setting chain is the one `sidebarLayout` already walks: `types.ts` (the union) →
      `content/settings.ts` (default + the `stored` reader at ~line 231 + the `input` sanitizer
      at ~line 342) → `admin/components/LayoutMenuFields.tsx` (it already owns the layout and
      menu block, and this is layout) → the six admin locale files → `web/admin/views.ts`.
      1. `home: { mode: 'list' | 'page' | 'front', page: string, listPath: string }`, defaulting
         to `{ mode: 'list', page: '', listPath: '/post' }`. **`list` must render byte-identical
         to today** — an existing install upgrades into no change at all, which is the whole
         constraint in ADR 0014. Pin that with a test, not with care.
      2. `app.ts`: `/` branches on the mode. `page` renders the chosen page through the SAME
         path `/{slug}` uses, so the reading view, the ToC rail and the cache key all behave.
         Guard the case where the slug is missing, unpublished or trashed: fall back to the
         list rather than 404, because a homepage that 404s is a dead site.
      3. Mount the list at `home.listPath` when the mode is not `list`. **Reserve the path**:
         it lives in the same `/{slug}` namespace as every post and page, so a site with a post
         at `/post` must be refused the collision (`ensureSlugFree` is the existing check) or
         one of the two silently wins. Leave `/page/:n` exactly where it is — deliberate, ADR
         0014.
      4. The chosen page keeps its own `/{slug}`, and a 301 from that slug to `/` is the SEO
         answer to two URLs holding one document. `server/redirects.ts` already does this.
      5. `renderSitemap` must stop claiming both, and the canonical on the list's page 1 moves
         from `/` to `listPath`.
      6. `warm.ts` warms posts and pages but never `/` — its comment says otherwise, which is a
         separate small lie to fix while here. In `page` mode the homepage is warmed for free
         because it IS a page; in `list` mode it still is not.

- [ ] **Homepage mode, part 2: the composed front page.** [ADR 0014](../docs/decisions/0014-homepage-modes.md).
      The big one. Do NOT start it until part 1 is live, because every row below renders inside
      the branch part 1 adds.
      1. **`home.front` settings**, roughly twenty options: `kind` (`image` | `text`), and per
         row `lead` / `featured` / `strips[]` / `popular` / `latest`, plus `meta.date`,
         `meta.readingTime` and `tagLinks`. `strips` is an ORDERED list of
         `{ category, count, columns }` and is the fiddliest admin surface in the feature —
         budget more for it than for any renderer.
      2. **A new renderer**, `web/front.ts` plus its own sheet. `renderListing` is not reusable:
         a row is not a card. Row types are lead, featured, category strip, most-viewed, latest;
         **the order is fixed in code** and settings only choose presence, counts and columns.
      3. **The card that carries an image does not exist yet** — `listing.ts`'s `card()` emits
         none. Write it against `featuredImage` falling back to `coverImage`, with the 1024/1600
         variants `media/image.ts` already produces. Decide and write down which field means
         what; today both exist and neither is displayed.
      4. **Both kinds from one grammar** (ADR 0014): `image` leads with a picture and a short
         standfirst, `text` drops it, raises the headline a step and lets the standfirst run.
         Not two layouts.
      5. **Three columns collapse 3 → 2 → 1**, and on mobile the headline sits ABOVE the image.
         This is the heaviest CSS in the feature and it is written once for every row.
      6. `tagLinks` derives the top tags of a strip's category from posts already in memory —
         a group-by, not a query. Hide the row under three tags.
      7. `popular` over 7 or 30 days needs a windowed query; `analytics/summary.ts`'s
         `getViewTotals` is all-time only.
      8. **It must look finished with the IDE chrome OFF**, which is the default. `// label` and
         `[literal]` are a layer a site may add, never the thing holding the design up.
      9. Zero JavaScript, and no new invalidation: `cached('/')` plus Invariant 1 plus the
