# `import-v1`: one-way importer from Quire 1.x
...
```
bun run import-v1 \
  --postgrest "https://.../rest/v1"  --token "$POSTGREST_TOKEN" \
...
would otherwise be the only reason it exists.
...
15. render_cache                warm every code block in every post and page
```

Step 15 is new. It runs `shiki` over every fenced block in the imported corpus and fills
`render_cache` (01-schema.md section 4). It is not strictly required, because the read
path self-heals on a miss, but without it the golden comparison in M2 measures an empty
cache and the first reader after cutover pays for every block on the site.
...
concatenate a canonical rendering of every column, hash. Computed by the same TypeScript
function on both sides, so type formatting cannot cause a false difference.
...
- **Any identity.** Google login is removed (parity exception #5). There is no user row
  to migrate: the owner account is created by `quire user create` and enrols TOTP at
  first sign-in. See 06-auth.md. `AUTHORIZED_EMAIL` becomes `users.email` by hand, once
- MCP tokens **are** imported, hash format preserved. Verify a live MCP write before the
  DNS switch; AI publishing failing silently is the failure mode this guards
